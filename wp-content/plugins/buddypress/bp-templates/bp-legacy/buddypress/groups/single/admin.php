...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<p>
		<label for="group-notify-members">
			<input type="checkbox" name="group-notify-members" value="1" /> <?php _e( 'Notify event members of these changes via email', 'buddypress' ); ?>
		</label>
	</p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
	...
	<div class="radio">
		<label>
			...
			<strong><?php _e( 'This is a public event', 'buddypress' ); ?></strong>
			<ul>
	...
	<div class="radio">
		<label>
			...
			<strong><?php _e( 'All event members', 'buddypress' ); ?></strong>
		</label>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<?php if ( bp_get_group_has_avatar() ) : ?>
				...
				<p><?php _e( "If you'd like to remove the existing event profile photo but not upload a new one, please use the delete event profile photo button.", 'buddypress' ); ?></p>

				<?php bp_button( array( 'id' => 'delete_group_avatar', 'component' => 'groups', 'wrapper_id' => 'delete-group-avatar-button', 'link_class' => 'edit', 'link_href' => bp_get_group_avatar_delete_link(), 'link_title' => __( 'Delete Event Profile Photo', 'buddypress' ), 'link_text' => __( 'Delete Event Profile Photo', 'buddypress' ) ) ); ?>
	...
	<div class="bp-widget">
		...
		<?php else: ?>
			...
			<div id="message" class="info">
				<p><?php _e( 'This event has no members.', 'buddypress' ); ?></p>
			</div>
...
<?php do_action( 'groups_custom_edit_steps' ) // Allow plugins to add custom event edit screens ?>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div id="message" class="info">
		<p><?php _e( 'WARNING: Deleting this event will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'buddypress' ); ?></p>
	</div>

	<label><input type="checkbox" name="delete-group-understand" id="delete-group-understand" value="1" onclick="if(this.checked) { document.getElementById('delete-group-button').disabled = ''; } else { document.getElementById('delete-group-button').disabled = 'disabled'; }" /> <?php _e( 'I understand the consequences of deleting this event.', 'buddypress' ); ?></label>

	<?php do_action( 'bp_after_group_delete_admin' ); ?>

	<div class="submit">
		<input type="submit" disabled="disabled" value="<?php esc_attr_e( 'Delete Event', 'buddypress' ); ?>" id="delete-group-button" name="delete-group-button" />
	</div>
